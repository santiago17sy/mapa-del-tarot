...
import { cardsQuery } from "@/lib/tarot";
import { supabase } from "@/integrations/supabase/client";

export const Route = createFileRoute("/_authenticated/tirada")({
  head: () => ({
    meta: [
      { title: "Mi primera tirada — Mapa del Tarot" },
      { name: "description", content: "Prepara, realiza e interpreta tu primera tirada de tres cartas." },
      { property: "og:title", content: "Mi primera tirada" },
      { property: "og:description", content: "Te acompañamos paso a paso en tu primera lectura." },
    ],
  }),
...
function ReadingPage() {
  ...
  async function finish() {
    ...
    await supabase.from("readings").insert({
      ...
      question,
      card_1: picked[0] || null,
      card_2: picked[1] || null,
      card_3: picked[2] || null,
  ...
  return (
    <div className="space-y-6">
      <SectionTitle kicker={`Paso ${Math.min(step, STEPS)} de ${STEPS}`}>🔮 Mi Primera Tirada</SectionTitle>
      <p className="text-sm text-muted-foreground">
        Te acompañaremos paso a paso. No necesitas memorizar todas las cartas para comenzar.
      </p>

      <div className="h-1.5 overflow-hidden rounded-full bg-secondary">
        <div className="h-full bg-gold" style={{ width: `${(Math.min(step, STEPS) / STEPS) * 100}%` }} />
      </div>
      ...
      {step === 3 ? (
        <StepCard title="Baraja y corta tu mazo">
          <p className="text-sm text-muted-foreground">
            ...
            mazo en tres montones con la mano izquierda y vuelve a unirlos en el orden que prefieras.
          </p>
      ...
      {step === 4 ? (
        <StepCard title="Elige 3 cartas">
          <p className="text-sm text-muted-foreground">
            Saca tres cartas de tu propio mazo físico y regístralas aquí.
          </p>
          {[0, 1, 2].map((index) => (
          ...
          <Primary disabled={picked.some((id) => !id)} onClick={() => setStep(5)}>
            Continuar
          </Primary>
        </StepCard>
      ) : null}
      ...
      {step === 5 ? (
        <StepCard title="Observa tus cartas">
          ...
          <p className="text-sm text-muted-foreground">
            Antes de leer significados, observa: símbolos, colores, personajes, hacia dónde miran y
      ...
      {step === 6 ? (
        <StepCard title="Interpreta tu tirada">
          {question ? (
            <p className="rounded-xl bg-secondary/60 p-3 text-sm text-primary">Tu pregunta: {question}</p>
          ) : null}
            ...
            {chosen.map((card) => (
              <div key={card!.id} className="rounded-xl border border-border p-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  >
                    {openCard === card!.id ? "Ocultar" : "Ver significado"}
                  </button>
                </div>
                {openCard === card!.id ? (
                  <div className="mt-3">
          ...
          {saved ? (
            <div className="space-y-3">
              <p className="text-sm text-primary">Tirada guardada ✦ Gracias por practicar.</p>
              <button
                onClick={() => {
                  setStep(1);
                  setQuestion("");
            ...
            <Primary onClick={finish}>Finalizar tirada</Primary>
          )}
        </StepCard>
      ) : null}
    </div>
